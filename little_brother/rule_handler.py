...
class RuleSetConfigModel(configuration.ConfigModel):
    ...
    def __str__(self):
        ...
        return "Rule set (user=%s, context=%s, time-of-day=[%s to %s], max-time-per-day:%s, max-duration=%s, min-break=%s)" % (
            self.username, self.context, min_time, max_time, max_time_per_day, max_duration, min_break)
...
class RuleSetSectionHandler(configuration.ConfigurationSectionHandler):
    ...
    def handle_section(self, p_section_name):
        ...
        rule_set_section.max_activity_duration = tools.get_string_as_duration(p_string=rule_set_section.max_activity_duration)
        rule_set_section.min_break = tools.get_string_as_duration(p_string=rule_set_section.min_break)
                
        
    def read_time_of_day(self, p_time_of_day):
...
class RuleHandler(object):
    ...
    def get_active_ruleset_config(self, p_username, p_reference_date):
        ...
        if ruleset_configs is not None:
            for c_config in ruleset_configs:
                ...
                active = context_rule_handler.is_active(p_reference_date=p_reference_date, p_context_details=c_config.context_details)
    ...
    def check_min_break(self, p_rule_set, p_stat_info, p_rule_result_info):
        ...
        if p_rule_set.min_break is not None and p_rule_set.max_activity_duration is not None:
            ...
            if p_stat_info.previous_activity_duration is not None:
                fraction_used = min(1.0 * p_stat_info.previous_activity_duration / p_rule_set.max_activity_duration, 1.0)
    ...
    def process_ruleset(self, p_stat_info, p_reference_time, p_rule_override, p_locale):
        ...
        if rule_set is not None:
            self.check_time_of_day(p_rule_set=rule_result_info.effective_rule_set, p_stat_info=p_stat_info, p_rule_result_info=rule_result_info)
            self.check_time_per_day(p_rule_set=rule_result_info.effective_rule_set, p_stat_info=p_stat_info, p_rule_result_info=rule_result_info)
