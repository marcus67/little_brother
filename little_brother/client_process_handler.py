...
class ClientProcessHandler(process_handler.ProcessHandler):
    ...
    def handle_event_kill_process(self, p_event):
        ...
        fmt = "Kill process %d of user %s on host %s with signal SIGHUP" % (p_event.pid, p_event.username, p_event.hostname)
        ...
        if len(alive) > 0:
            fmt = "Kill process %d of user %s on host %s with signal SIGKILL" % (p_event.pid, p_event.username, p_event.hostname)
            self._logger.debug(fmt)
            ...
            try:
                ...
                if uids.real in p_uid_map:                  
                    ...
                    if p_process_regex_map[username].match(proc_name):
                        start_time = datetime.datetime.fromtimestamp(proc.create_time(), datetime.timezone.utc).astimezone().replace(tzinfo=None)           
