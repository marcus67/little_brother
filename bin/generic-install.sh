...
${PIP3} download -d $LIB_DIR --no-deps little_brother==0.4.27
...
if [ ! -f $LIB_DIR/little-brother-0.4.27.tar.gz ] ; then
  echo "ERROR: package little-brother-0.4.27.tar.gz not found in $LIB_DIR!"
  echo "Download from test.pypi.org and execute again."
  exit 2
else
  echo "Package little-brother-0.4.27.tar.gz was found."
fi
...
echo "  * little-brother-0.4.27.tar.gz"
...
${PIP3} install --upgrade --ignore-installed \
     ${LIB_DIR}/little-brother-0.4.27.tar.gz\
     ${LIB_DIR}/python-base-app-0.2.45.tar.gz\
     ${LIB_DIR}/some-flask-helpers-0.2.3.tar.gz


echo "Removing installation file ${LIB_DIR}/little-brother-0.4.27.tar.gz..."
rm ${LIB_DIR}/little-brother-0.4.27.tar.gz
